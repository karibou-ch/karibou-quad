...
with open('../tests/data/orders.json') as json_file:
    ...
    axis = []
    # group by year
    for year, items in groupby(sorted(mapped_items, key=lambda mi: mi['date']), key=lambda mi: mi['date'].year):
        items = list(items)
    ...
    plt.suptitle("karibou.ch",  fontsize=8)
    ...
    plt.savefig('./' + vendors_directory + '/' + "karibou.ch" + '.png',dpi=170)
    plt.clf()
    plt.close()
